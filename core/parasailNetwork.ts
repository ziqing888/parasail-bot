...
export class ParasailNetwork {
  ...
  private async makeRequest(method: string, url: string, config: AxiosRequestConfig = {}, retries: number = 3): Promise<AxiosResponse> {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        logMessage(`请求失败: ${url} - ${error.message} (状态码: ${error.response?.status || "未知"})`, "错误");
  ...
  private async setupIntervals(): Promise<void> {
    ...
    logMessage(`API 返回的统计数据: ${JSON.stringify(stats?.data)}`, "调试");
    if (!stats?.data?.data) return; // 检查嵌套的 data

    this.showStats(stats.data.data.points, stats.data.data.last_checkin_time);
    this.statsInterval = setInterval(async () => {
      const newStats = await this.makeRequest("GET", `${API_BASE_URL}/api/v1/node/node_stats?address=${this.wallet.address}`, {
        headers: { Authorization: `Bearer ${this.currentToken}` },
      });
      if (newStats?.data?.data) this.showStats(newStats.data.data.points, newStats.data.data.last_checkin_time);
    }, 60 * 60 * 1000);

    const lastCheckin = stats.data.data.last_checkin_time ?? Math.floor(Date.now() / 1000);
  ...
  private showStats(points: number | undefined, lastCheckin: number | undefined): void {
    const line = "=".repeat(50);
    const safePoints = points !== undefined ? points : "未知";
